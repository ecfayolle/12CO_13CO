...
def zeroth_fit(File,deca,molec,Nml,Eth,nu_des_ini,t_start,t_stop):
	...
	T=T_tot[np.where((T_tot>t_start-deca) & (T_tot<t_stop-deca))]
	N=N_tot[np.where((T_tot>t_start-deca) & (T_tot<t_stop-deca))]
	N_err=1e10+N*0
	T_err=T*0.0+errort
...
#		return p[1] +t*p[0]
	...
	E_des=-p[0]
	nu_des=h_rate*np.exp(np.log(np.sqrt(2.0*1e19*8.31*-1*p[0]/(np.pi*np.pi*m_mol))/h_rate))
	...
	print 'E_des theo', Eth
	#print 'nu fit e12', 1e-12*h_rate*np.exp(p[1]), '+-',err[1]*1e-12*h_rate*np.exp(p[1])
	print 'nu fit e12', 1e-12*h_rate*np.exp(np.log(np.sqrt(2.0*1e19*8.31*-1*p[0]/(np.pi*np.pi*m_mol))/h_rate))
	print 'nu formu', 1e-12*nu_des_ini
	print 'nu theo', 1e-12*nu_theo
	...
	plot_tpd=File+'_reglinfit.pdf'
	plt.errorbar(x,y,xerr=x_err,yerr=y_err,color='k')
	#plt.errorbar(a,b,yerr=c, linestyle="None")
	#plt.plot(x,p[1]+p[0]*x,color='r',linestyle='--',linewidth=3.0)
	plt.plot(x,np.log(np.sqrt(2.0*1e19*8.31*-1*p[0]/(np.pi*np.pi*m_mol))/h_rate)+p[0]*x,color='r',linestyle='--',linewidth=3.0)
	plt.text(xtex,0.0,'-Slope: '+str(round(E_des,2))+' +- '+str(round(err[0],2)))
	#plt.text(0.04,-0.5,'exp(Intercept)*h_rate: '+ nu_des)
	...
	plt.plot(T_tot,np.clip(k_d_ml,0,np.max(N_tot/1e15)))
	plt.plot(T_tot,np.clip(k_d_mode,0,np.max(N_tot/1e15)),color='r')
	plt.xlim(15,50)
	plt.savefig(plot_tpd)
	plt.close('all')